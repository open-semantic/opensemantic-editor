...
const DiagramViewInner = () => {
  ...
  const updateModel = useCallback((updates) => {
    ...
    setYaml(stringifyYaml(newData));
  }, [parsedData, semanticModel, setYaml]);
  ...
  const handleNodesChange = useCallback((changes) => {
    ...
    if (hasPositionChange && modelName) {
      clearTimeout(saveTimerRef.current);
      saveTimerRef.current = setTimeout(() => {
        setNodes(cur => { saveDiagramPositions(modelName, cur); return cur; });
      }, 500);
    }
  }, [onNodesChange, modelName, setNodes]);

  // Handle adding a new field to a dataset
  const handleAddField = useCallback((nodeId, newField) => {
    const datasetIndex = parseInt(nodeId.replace('dataset-', ''));
    const updatedDatasets = [...datasets];
    if (updatedDatasets[datasetIndex]) {
      const dataset = updatedDatasets[datasetIndex];
      updatedDatasets[datasetIndex] = {
        ...dataset,
        fields: [...(dataset.fields || []), newField],
      };
      updateModel({ datasets: updatedDatasets });
    }
  }, [datasets, updateModel]);

  // Handle deleting a field from a dataset
  const handleDeleteField = useCallback((nodeId, fieldIndex) => {
    const datasetIndex = parseInt(nodeId.replace('dataset-', ''));
    const updatedDatasets = [...datasets];
    if (updatedDatasets[datasetIndex]) {
      const dataset = updatedDatasets[datasetIndex];
      ...
      updatedDatasets[datasetIndex] = { ...dataset, fields };
      updateModel({ datasets: updatedDatasets });
    }
  }, [datasets, updateModel]);

  // Handle reordering a field within a dataset
  const handleReorderField = useCallback((nodeId, fromIndex, toIndex) => {
  ...
  // Handle updating a dataset
  const handleUpdateDataset = useCallback((nodeId, updatedDataset) => {
    const datasetIndex = parseInt(nodeId.replace('dataset-', ''));
    const updatedDatasets = [...datasets];
    if (updatedDatasets[datasetIndex]) {
  ...
  const handleDeleteDataset = useCallback((nodeId) => {
    const datasetIndex = parseInt(nodeId.replace('dataset-', ''));
    const updatedDatasets = [...datasets];
  ...
  const handleAddDataset = useCallback(() => {
    ...
    updateModel({ datasets: [...datasets, newDataset] });
  }, [datasets, updateModel]);

  // Handle showing field details in drawer
  const handleShowFieldDetails = useCallback((nodeId, fieldIndex, nodePosition, fieldOffset, openMethod = 'hover') => {
    if (openMethod === 'hover') return;
    ...
    setSelectedField({
      ...
      datasetIndex,
      fieldIndex,
      fieldPath: `semantic_model[0].datasets[${datasetIndex}].fields[${fieldIndex}]`,
    });
  }, [datasets]);
  ...
  useEffect(() => {
    const handleKeyDown = (event) => {
      ...
      if (event.key === 'Delete' || event.key === 'Backspace') {
        ...
        const { datasetIndex, fieldIndex } = selectedField;
        if (typeof fieldIndex === 'number' && typeof datasetIndex === 'number') {
          if (window.confirm('Are you sure you want to delete this field?')) {
            handleDeleteField(`dataset-${datasetIndex}`, fieldIndex);
            setSelectedField(null);
          }
        }
      }
  ...
  // Handle field connections — creates OSI relationship
  const onConnect = useCallback((connection) => {
    // Parse handle IDs: dataset-{idx}-field-{idx}-source/target
    const sourceMatch = connection.sourceHandle?.match(/dataset-(\d+)-field-(\d+)-source/);
    const targetMatch = connection.targetHandle?.match(/dataset-(\d+)-field-(\d+)-target/);
    if (!sourceMatch || !targetMatch) return;

    const sourceDatasetIndex = parseInt(sourceMatch[1]);
    const sourceFieldIndex = parseInt(sourceMatch[2]);
    const targetDatasetIndex = parseInt(targetMatch[1]);
    ...
    // Find existing relationship between these two datasets
    const existingIdx = updatedRelationships.findIndex(
      r => r.from === sourceDataset.name && r.to === targetDataset.name
    );

    if (existingIdx >= 0) {
      // Add column pair to existing relationship
      const rel = { ...updatedRelationships[existingIdx] };
      const fromCols = [...(rel.from_columns || [])];
      const toCols = [...(rel.to_columns || [])];

      // Check if this pair already exists
      const pairExists = fromCols.some((fc, i) => fc === sourceField.name && toCols[i] === targetField.name);
      if (!pairExists) {
        fromCols.push(sourceField.name);
        toCols.push(targetField.name);
        rel.from_columns = fromCols;
        rel.to_columns = toCols;
        updatedRelationships[existingIdx] = rel;
        updateModel({ relationships: updatedRelationships });
      }
    } else {
      // Create new relationship
      const relName = `${sourceDataset.name}_to_${targetDataset.name}`;
      updatedRelationships.push({
        name: relName,
        from: sourceDataset.name,
        to: targetDataset.name,
        from_columns: [sourceField.name],
        to_columns: [targetField.name],
      });
      updateModel({ relationships: updatedRelationships });
    }
  ...
  useEffect(() => {
    ...
    // On first load, try localStorage positions, then fall back to ELK
    const savedPositions = !hasInitialLayout.current
      ? getDiagramPositions(modelName)
      : null;
      ...
      .map((dataset, index) => {
        ...
        return {
          ...
          data: {
            ...
            onShowFieldDetails: handleShowFieldDetails,
            openFieldDetails: selectedField?.nodeId === `dataset-${index}` ? {
    ...
    relationships.forEach((rel) => {
      ...
      if (sourceDatasetIndex === -1 || targetDatasetIndex === -1) return;
        ...
        for (let i = 0; i < pairCount; i++) {
          ...
          if (sourceFieldIndex !== -1 && targetFieldIndex !== -1) {
            fieldEdges.push({
              ...
              targetHandle: `dataset-${targetDatasetIndex}-field-${targetFieldIndex}-target`,
              type: 'default',
              selectable: true,
              deletable: true,
              focusable: true,
    ...
    const applyNodes = (datasetNodes) => {
      ...
      setNodes(currentNodes => {
        ...
        if (currentNodes.length === 0) {
          ...
          saveDiagramPositions(modelName, datasetNodes);
          return datasetNodes;
        }
        ...
        const result = datasetNodes.map(dn => {
          ...
          return { ...dn, position: { x: maxRight + 80, y: centerY } };
        });
        saveDiagramPositions(modelName, result);
        return result;
      });
    };
  ...
  // Focus on selected dataset when coming from sidebar
  useEffect(() => {
    const focusDatasetIndex = location.state?.focusDatasetIndex;
    if (focusDatasetIndex != null &&
        reactFlowInstance &&
        nodes.length > 0 &&
        lastFocusedIndexRef.current !== focusDatasetIndex) {

      const nodeId = `dataset-${focusDatasetIndex}`;
  ...
  }, [location.state?.focusDatasetIndex, reactFlowInstance, nodes, setNodes]);
    ...
    <div className="w-full h-full">
      ...
      >
        ...
        <Controls showZoom={false} showFitView={false} showInteractive={false}>
          <ControlButton onClick={handleAddDataset} title="Add Dataset">
          ...
          <Panel position="top-right" className="bg-white rounded-lg shadow-lg p-3 max-w-xs">
            <div className="bg-blue-50 border border-blue-200 rounded text-xs space-y-2 p-3">
              ...
              <ul className="text-blue-800 space-y-1 list-disc list-inside">
                <li>Click on dataset or field names to edit</li>
                <li>Click on fields to see details in drawer</li>
        ...
        {datasets.length === 0 && (
          <Panel position="center" className="pointer-events-auto">
            <div className="bg-white rounded-lg shadow-lg p-8 text-center">
              ...
              <h3 className="mt-3 text-sm font-semibold text-gray-900">No datasets</h3>
              <p className="mt-2 text-sm text-gray-500">Get started by creating your first dataset.</p>
              <div className="mt-6">
                <button
